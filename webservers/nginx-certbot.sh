...
# Spust certberus hooks renewed.d a post-deploy.d
...
stage_issue_cert() {
    ...
    # Pred samotnym certbot requestem:
    #  - odstran nase self-signed placeholdery (certbot by odmitl s "live directory exists")
    #  - uklid osirotely certbot stav z predchozich selhani (prazdne renewal confs apod.)
    local dd
    for dd in "${VALID_DOMAINS[@]}"; do
        if [[ -e "/etc/letsencrypt/live/$dd/.certberus-placeholder" ]]; then
            cb_log "Odstranuji placeholder /etc/letsencrypt/live/$dd/ pred certbot requestem"
