...
stage_issue_cert() {
    ...
    for d in "${VALID_DOMAINS[@]}"; do
        ...
        # Kopie do SSL dir pro Tomcat
        if [[ "$CB_DRY_RUN" == "0" ]]; then
            mkdir -p "$CB_TOMCAT_SSL_DIR/$d"
            cp "/etc/letsencrypt/live/$d/fullchain.pem" "$CB_TOMCAT_SSL_DIR/$d/" || cb_die "Nelze zkopirovat fullchain.pem pro $d do $CB_TOMCAT_SSL_DIR"
            cp "/etc/letsencrypt/live/$d/privkey.pem" "$CB_TOMCAT_SSL_DIR/$d/" || cb_die "Nelze zkopirovat privkey.pem pro $d do $CB_TOMCAT_SSL_DIR"
            [[ -s "$CB_TOMCAT_SSL_DIR/$d/fullchain.pem" && -s "$CB_TOMCAT_SSL_DIR/$d/privkey.pem" ]] || cb_die "Kopie certifikatu pro $d is empty/neuplna"
